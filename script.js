...
function renderCartItems() {
    ...
    // Скрываем формы оплаты и сообщение об успехе
    document.getElementById('payment-methods').style.display = 'block';
    document.getElementById('card-payment-form').style.display = 'none';
    document.getElementById('sbp-payment-form').style.display = 'none';
    document.getElementById('payment-success').style.display = 'none';
}
...
function selectPaymentMethod(method) {
    document.getElementById('card-payment-form').style.display = method !== 'sbp' ? 'block' : 'none';
    document.getElementById('sbp-payment-form').style.display = method === 'sbp' ? 'block' : 'none';
    
    // Сбрасываем выбранные методы
    document.querySelectorAll('.payment-option').forEach(option => {
        option.classList.remove('selected');
    });
...
function processSBPPayment() {
    // Здесь должна быть логика обработки СБП платежа
    // В демо-версии просто показываем сообщение об успехе
    showPaymentSuccess();
}
